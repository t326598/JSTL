...
public class PersistenceLoginsDAO extends JDBConnection {
	...
	public Persistence_logins insert(String username) {
		...
		try {
			...
			System.err.println("자옫 로그인 정보 등록 중, 예외 발생");
			e.printStackTrace();
		...
		Persistence_logins logins = Persistence_logins.builder()
														.username(username)
														.token(token)
	...
	public Persistence_logins select(String username) {
		...
		String sql = "SELECT *"
				...
				+ "WHERE username = ?";
		Persistence_logins logins = null;
		...
		try {
			...
			if(rs.next()) {
				logins = Persistence_logins.builder()
											.username(username)
